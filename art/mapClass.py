...
class track:
  ...
  def create_polygons(self):
    ...
    self.top = [(0,0),                          #static so not drawn, for collision only
                (0,screen_height/10),
  ...
  def circle_points_a(self,amount, radius,cx,cy,corner,array): #top left curve
    angle = 0
  ...
  def circle_points_b(self,amount, radius,cx,cy,corner,array): #top right curve
    angle = 0
    for i in xrange(1,amount+2):
  ...
  def circle_points_c(self,amount, radius,cx,cy,corner,array):  #bottom right curve
    angle = 0
    for i in xrange(1,amount+2):
      x = cx+(cos(angle)*radius) 
      y = cy+(sin(angle)*radius)
  ...
  def circle_points_d(self,amount, radius,cx,cy,corner,array): #bottom left curve
    angle = 0
    for i in xrange(1,amount+2):
      x = cx-(sin(angle)*radius) 
      y = cy+(cos(angle)*radius)
